...
void hooks::run_command::run_command_detour( void* ecx, void* edx, sdk::c_base_player* player, sdk::c_user_cmd* command, void* move_helper )
{
	...
	simulation_ticks      = 1 + g_interfaces.net_channel->get_drop_number( );

	int ideal_final_tick     = g_interfaces.globals->tick_count + correction_ticks + time_to_ticks( g_interfaces.net_channel->get_latency( 0 ) );
