...
// this is server side only, it's used to mark properties whose send_proxy_* functions encode against gp_globals->tickcount (the only ones that currently do this are
...
#define extern_send_table(table_name)	namespace table_name {extern send_table g_send_table;}
#define extern_recv_table(table_name)	namespace table_name {extern recv_table g_recv_table;}
...
{
public:
	d_variant()				{m_type = dpt_float;}
	d_variant(float val)		{m_type = dpt_float; m_float = val;}
