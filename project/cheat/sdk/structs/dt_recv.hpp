...
#define begin_recv_table_nobase(struct_name, table_name) \
	template <typename t> int clientstruct_init(t *); \
	namespace table_name { \
		struct ignored; \
	} \
	template <> int clientstruct_init<table_name::ignored>(table_name::ignored *); \
	namespace table_name {	\
	...
	{ \
		...
		static recv_prop recv_props[] = { \
			recv_prop_int("should_never_see_this", 0, sizeof(int)),		// it adds a dummy property at the start so you can define "empty" send_tables.
...
#define recvinfo_name(var_name, remote_var_name)	#remote_var_name, offsetof(current_recv_d_tstruct, var_name), sizeof(((current_recv_d_tstruct*)0)->var_name)
...
#define recvinfo_array(var_name)					#var_name, offsetof(current_recv_d_tstruct, var_name), sizeof(((current_recv_d_tstruct*)0)->var_name[0]), sizeof(((current_recv_d_tstruct*)0)->var_name)/sizeof(((current_recv_d_tstruct*)0)->var_name[0])
...
recv_prop recv_prop_float(
	const char *p_var_name,
	int offset,
	int sizeof_var=-1,	// handled by recvinfo macro, but set to sizeof_ignore if you don't want to bother.
	int flags=0,
	recv_var_proxy_fn var_proxy=recv_proxy_float_to_float
);

recv_prop recv_prop_vector(
	const char *p_var_name,
	int offset,
	int sizeof_var=-1,	// handled by recvinfo macro, but set to sizeof_ignore if you don't want to bother.
	int flags=0,
	recv_var_proxy_fn var_proxy=recv_proxy_vector_to_vector
);

recv_prop recv_prop_vector_xy(
	const char *p_var_name,
	int offset,
	int sizeof_var=-1,	// handled by recvinfo macro, but set to sizeof_ignore if you don't want to bother.
	int flags=0,
	recv_var_proxy_fn var_proxy=recv_proxy_vector_xy_to_vector_xy
);
...
recv_prop recv_prop_int(
	const char *p_var_name,
	int offset,
	int sizeof_var=sizeof_ignore,	// handled by recvinfo macro, but set to sizeof_ignore if you don't want to bother.
	int flags=0,
	recv_var_proxy_fn var_proxy=0
);

recv_prop recv_prop_string(
	const char *p_var_name,
	int offset,
	int buffer_size,
	int flags=0,
...
recv_prop recv_prop_data_table(
	const char *p_var_name,
	int offset,
	int flags,
	recv_table *p_table,
...
recv_prop recv_prop_array3(
	const char *p_var_name,
	int offset,
	int sizeof_var,
	int elements,
	recv_prop p_array_prop,
	data_table_recv_var_proxy_fn var_proxy=data_table_recv_proxy_static_data_table
);
...
recv_prop internal_recv_prop_array(
	const int element_count,
	const int element_stride,
	const char *p_name,
	array_length_recv_proxy_fn proxy
);
...
#define recv_prop_variable_length_array( array_length_proxy, var_template, array_name )			\
	var_template,										\
	internal_recv_prop_array(								\
		sizeof(((current_recv_d_tstruct*)0)->array_name) / propsizeof(current_recv_d_tstruct, array_name[0]), \
		propsizeof(current_recv_d_tstruct, array_name[0]),	\
		#array_name,										\
		array_length_proxy								\
		)
...
// use this and pass the array name and it will figure out the count and stride automatically.
#define recv_prop_array( var_template, array_name )			\
	recv_prop_variable_length_array( 0, var_template, array_name )
...
#define recv_prop_array2( array_length_proxy, var_template, element_count, element_stride, array_name )		\
	var_template,																	\
	internal_recv_prop_array( element_count, element_stride, #array_name, array_length_proxy )
