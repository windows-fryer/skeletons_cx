...
void menu::window::draw( int& window_x, int& window_y, int& size_x, int& size_y )
{
	...
	g_render.render_filled_rectangle( window_x, window_y, size_x, size_y, sdk::color( 12, 12, 12 ) );
	g_render.render_rectangle( window_x + 1, window_y + 1, size_x - 3, size_y - 3, sdk::color( 30, 30, 30 ) );

	g_render.render_filled_rectangle( window_x + 3, window_y + 3, size_x - 6, 20, sdk::color( 20, 20, 20 ) );
	...
	g_render.render_filled_rectangle( window_inset.x, window_inset.y, size_x - 7, 20, sdk::color( 20, 20, 20 ) );
	g_render.render_rectangle( window_inset.x, window_inset.y, size_x - 7, 20, sdk::color( 30, 30, 30 ) );
	...
	if ( selected_color_picker ) {
		auto& option = g_config.find< sdk::color >( selected_color_picker );
		...
		for ( int iterator = 0; iterator < 128; iterator++ ) {
			...
			sdk::hsv color_from_hsv{ option.to_hsv( ).h, 0, vibrance, 255 };
			sdk::hsv color_to_hsv{ option.to_hsv( ).h, 100, vibrance, 255 };

			sdk::color color_from = color_from_hsv.to_color( );
			sdk::color color_to   = color_to_hsv.to_color( );
			...
			sdk::hsv color_alt_hsv{ hue, 100, 100, 255 };

			sdk::color color_alt = color_alt_hsv.to_color( );
			...
			sdk::hsv color_alt_alt_hsv{ option.to_hsv( ).h, 100, 100, alpha };

			sdk::color color_alt_alt = color_alt_alt_hsv.to_color( );
		...
		g_render.render_text( color_picker_x + 3.f + 145.f / 4.f, color_picker_y + 3.f + 129.f + font_size.y / 2,
		                      font_alignment::AL_HORIZONTAL_CENTER | font_alignment::AL_VERTICAL_CENTER, font_flags::FLAG_NONE, "copy", menu_font,
		...
		g_render.render_text( color_picker_x + 3.f + 145.f / 2 + 145.f / 4.f, color_picker_y + 3.f + 129.f + font_size.y / 2,
		                      font_alignment::AL_HORIZONTAL_CENTER | font_alignment::AL_VERTICAL_CENTER, font_flags::FLAG_NONE, "paste", menu_font,
		                      sdk::color( 255, 255, 255 ) );
		...
		g_render.set_viewport( { static_cast< float >( color_picker_x + 3 ), static_cast< float >( color_picker_y + 3 ) }, { 128, 128 } );
...
void menu::window::input( int& window_x, int& window_y, int& size_x, int& size_y )
{
	...
	if ( selected_color_picker && g_input.key_state< input::KEY_RELEASED >( VK_LBUTTON ) &&
	     !inside_position( { static_cast< float >( mouse_x ), static_cast< float >( mouse_y ) },
	                       { static_cast< float >( color_picker_x ), static_cast< float >( color_picker_y ) }, { 152, 152 } ) ) {
	...
	if ( selected_color_picker ) {
		...
		if ( g_input.key_state< input::KEY_DOWN >( VK_LBUTTON ) && !dragging_hue_color_picker && !dragging_alpha_color_picker ) {
			if ( inside_position( { static_cast< float >( mouse_x ), static_cast< float >( mouse_y ) },
			                      { static_cast< float >( color_picker_x + 3 ), static_cast< float >( color_picker_y ) }, { 128, 128 } ) ||
			     dragging_picker_color_picker ) {
				...
				sdk::hsv buffer_hsv = option.to_hsv( );
				option =
					sdk::hsv{ buffer_hsv.h, static_cast< int >( 100 * saturation ), static_cast< int >( 100 * vibrance ), buffer_hsv.a }.to_color( );
			}
		} else {
		...
		if ( g_input.key_state< input::KEY_DOWN >( VK_LBUTTON ) && !dragging_picker_color_picker && !dragging_alpha_color_picker ) {
			if ( inside_position( { static_cast< float >( mouse_x ), static_cast< float >( mouse_y ) },
			                      { static_cast< float >( color_picker_x + 132 ), static_cast< float >( color_picker_y + 3 ) }, { 8, 128 } ) ||
			     dragging_hue_color_picker ) {
				...
				sdk::hsv buffer_hsv = option.to_hsv( );
				option              = sdk::hsv{ static_cast< int >( 360.f * hue ), buffer_hsv.s, buffer_hsv.v, buffer_hsv.a }.to_color( );
			}
		} else {
		...
		if ( g_input.key_state< input::KEY_DOWN >( VK_LBUTTON ) && !dragging_picker_color_picker && !dragging_hue_color_picker ) {
			if ( inside_position( { static_cast< float >( mouse_x ), static_cast< float >( mouse_y ) },
			                      { static_cast< float >( color_picker_x + 141 ), static_cast< float >( color_picker_y + 3 ) }, { 8, 128 } ) ||
				...
				sdk::hsv buffer_hsv = option.to_hsv( );
				option              = sdk::hsv{ buffer_hsv.h, buffer_hsv.s, buffer_hsv.v, static_cast< int >( 255.f * alpha ) }.to_color( );
			}
		} else {
		...
		if ( g_input.key_state< input::KEY_DOWN >( VK_LBUTTON ) ) {
			if ( inside_position( { static_cast< float >( mouse_x ), static_cast< float >( mouse_y ) },
			                      { static_cast< float >( color_picker_x + 3 ), static_cast< float >( color_picker_y + 3 + 129 ) },
			                      { 145 / 2 - 1, 16 } ) ) {
				copied_color = option;
			}
			if ( inside_position( { static_cast< float >( mouse_x ), static_cast< float >( mouse_y ) },
			                      { static_cast< float >( color_picker_x + 4 + 145 / 2 ), static_cast< float >( color_picker_y + 3 + 129 ) },
			                      { 145 / 2, 16 } ) ) {
				option = copied_color;
			}
		}
	...
	if ( g_input.key_state< input::KEY_DOWN >( VK_LBUTTON ) && !dragging_picker_color_picker && !dragging_hue_color_picker &&
	     !dragging_alpha_color_picker ) {
		if ( inside_position( g_input.mouse.pos, { static_cast< float >( window_x ), static_cast< float >( window_y ) },
		                      { static_cast< float >( size_x ), static_cast< float >( size_y ) } ) &&
		         allowed_to_drag && !dragging ||
		     resizing ) {
			if ( inside_position( g_input.mouse.pos,
			                      { static_cast< float >( window_x + size_x - 10 ), static_cast< float >( window_y + size_y - 10 ) }, { 10, 10 } ) ||
			     resizing ) {
				resizing = true;
				...
				window_x = std::clamp( window_x, 0, static_cast< int >( g_globals.screen_size.x ) - size_x );
				window_y = std::clamp( window_y, 0, static_cast< int >( g_globals.screen_size.y ) - size_y );
			}
		}
	} else {
		resizing = false;
...
bool menu::window::inside_position( sdk::vector mouse_position, sdk::vector window_position, sdk::vector window_size )
{
