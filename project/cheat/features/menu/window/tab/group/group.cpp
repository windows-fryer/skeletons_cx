...
void menu::group::draw( int& window_x, int& window_y, int& size_x, int& size_y, int current_index )
{
	...
	if ( position & static_cast< int >( group_position::GROUP_LEFT ) ) {
		...
		g_render.render_text( group_position_x + group_with_half, group_position_y + static_cast< int >( font_size.y ) / 2,
		                      font_alignment::AL_HORIZONTAL_CENTER | font_alignment::AL_VERTICAL_CENTER, font_flags::FLAG_NONE, name.c_str( ),
		...
		g_render.set_viewport( { static_cast< float >( group_position_x ), static_cast< float >( group_position_y ) },
		                       { static_cast< float >( group_width ), static_cast< float >( group_height ) } );
...
void menu::group::input( int& window_x, int& window_y, int& size_x, int& size_y, int current_index )
{
	...
	if ( position & static_cast< int >( group_position::GROUP_LEFT ) ) {
		...
		if ( menu::window::inside_position( { mouse_x, mouse_y },
		                                    { static_cast< float >( group_position_x ), static_cast< float >( group_position_y ) },
		                                    { static_cast< float >( group_width ), static_cast< float >( group_height ) } ) &&
		...
		g_render.set_viewport( { static_cast< float >( group_position_x ), static_cast< float >( group_position_y + 1 ) },
		                       { static_cast< float >( group_width ), static_cast< float >( group_height ) } );
